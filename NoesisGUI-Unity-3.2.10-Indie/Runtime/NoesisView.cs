                        ...
                        builder.SetRenderFunc((PassData passData, UnsafeGraphContext context) =>
                        {
                            ...
                            if (passData.cameraData.xrRendering)
                            {
                            ...
                            #endif
                                {
                                    ...
                                    NoesisRenderer.RenderOnscreen_(passData.view._uiView, viewProj0, viewProj1,
            ...
            if (_view._uiView != null && _view._visible)
            {
                ...
                if (renderingData.cameraData.xrRendering)
                {
                  ...
                  #endif
                    {
                        ...
                        NoesisRenderer.RenderOnscreen(_view._uiView, viewProj0, viewProj1,
...
#region High Definition Render Pipeline
    ...
    private void OnExecuteCustomPass(CommandBuffer commands, bool flipY)
    {
        if (_uiView != null && _visible)
        {
            // In HDRP, the custom pass is configured to always clear the stencil buffer before execution
            commands.BeginSample(_customSampler);
            NoesisRenderer.RenderOnscreen(_uiView, flipY, commands, true, false);
    ...
    private void RenderOnScreen(bool flipY, CommandBuffer commands)
    {
        if (_camera != null && _camera.stereoEnabled)
        {
            ...
            {
                NoesisRenderer.RenderOnscreen(_uiView,
    ...
    private static readonly Dictionary<int, bool> _eyeTextureCache = new Dictionary<int, bool>();

    private bool IsEyeTexture(RenderTexture texture)
    {
        int id = texture.GetInstanceID();

        if (_eyeTextureCache.TryGetValue(id, out bool cached))
            return cached;

        // In VR the Swap Chain is named 'XR Texture[#]' (before Unity 2020 it was 'RTDeviceEyeTextureArray')
        // In the Built-In pipeline there’s no public API to detect this reliably
        string name = texture.name;
        bool isEye = name.StartsWith("XR Texture") || name.StartsWith("RTDeviceEyeTextureArray");
        _eyeTextureCache[id] = isEye;
        return isEye;
    }
